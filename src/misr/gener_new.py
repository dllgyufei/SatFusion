...
class PhysMTMSynth:
    def __init__(self, n_frames=8, target_size: tuple = (40, 40), psf_sigma_px=(1.2,1.8),
                 shift_range=(-3,3), read_noise_std=0.001, shot_noise_gain=500):
    ...
    def _downsample(self, img):
        ...
        return F.interpolate(img, size=self.target_size, mode="area")

    def generate(self, hrms: torch.Tensor) -> torch.Tensor:
        """
        hrms: [B,1,C,H,W]  (原始 HR-MS)
