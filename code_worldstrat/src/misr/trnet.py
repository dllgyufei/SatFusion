...
class TRNet(nn.Module):
    ...
    def __init__(self, config, out_channels, use_pan):
    ...
    def forward(self, x, pan_feat, K, alphas=None, maps=None):
        ...
        x = x.permute(0, 3, 4, 1, 2).reshape(-1, x.shape[1],
                                               x.shape[2])
        if self.use_pan == True:
            x = self.superres(x, pan_feat=pan_feat, K=K)
        else:
            x = self.superres(x, pan_feat=None, K=K)
