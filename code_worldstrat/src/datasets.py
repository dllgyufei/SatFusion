...
class SatelliteDataset(Dataset):
    ...
    def compute_median_std(self, name=None):
        ...
        channels_over_all_revisits = torch.cat(
            [
                #x.to(float)
                (torch.as_tensor(x, dtype=torch.float32) / SPOT_MAX_EXPECTED_VALUE_12_BIT)# 🔹 numpy → torch.Tensor
...
def generate_scene_classification_filters(
    ...
    """
    # print("zhuyi:")
    # print(scene_classification_filter_threshold, scene_classification_filter_values)
...
if __name__ == "__main__":
    ...
    root_lr = '/home/tongyufei/SatFusion/dateset_entire/lr_dataset/*/L2A'
    root_hr = '/home/tongyufei/SatFusion/dateset_entire/hr_dataset/*/'
    subdir = ""
    file_postfix_lr = '-L2A_data.tiff'
    file_postfix_hr = '_ps.tiff'
    file_reader = None
    use_cache = True
    use_tifffile_as_reader = False
    pan_dataset = None
    ...
    # 🔹 如果有 list_of_aois 参数，读取 CSV 得到 AOI 列表
    list_of_aois = '/home/tongyufei/SatFusion/Code_WWW/pretrained_model/example.csv'
    if list_of_aois is not None:
        list_of_aois = list(pd.read_csv(list_of_aois, index_col=1).index)
    multiprocessing_manager = Manager()
    # dataset_lr = SatelliteDataset(
    #     root=root_lr, subdir="", number_of_revisits=8, file_postfix=file_postfix_lr, list_of_aois=list_of_aois, multiprocessing_manager=multiprocessing_manager
    # )
    # print(f"S2 median / std: {dataset_lr.compute_median_std()}")

    dataset_hr = SatelliteDataset(
        root=root_hr, subdir="", number_of_revisits=1, file_postfix=file_postfix_hr, list_of_aois=list_of_aois, multiprocessing_manager=multiprocessing_manager
    )
