...
from src.sharp.mamba.refine_mamba import Refine
def to_3d(x):
...
class CrossMamba(nn.Module):
    ...
    #    ms = global_f.transpose(1, 2).view(B, C, 128*8, 128*8)
        ms = global_f.transpose(1, 2).view(B, C, 156, 156)
...
class HinResBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        return x+resi
class Net_With_MISR(nn.Module):
    def __init__(self,num_channels=None,base_filter=None,args=None):
        super(Net_With_MISR, self).__init__()
        base_filter=32
        ...
        self.ms_encoder = nn.Sequential(nn.Conv2d(3,base_filter,3,1,1),HinResBlock(base_filter,base_filter),HinResBlock(base_filter,base_filter),HinResBlock(base_filter,base_filter))
        ...
        self.output = Refine(base_filter,3)
    def forward(self,ms,_,pan):

        #ms_bic = F.interpolate(ms,scale_factor=1)
        ms_bic = ms
    ...
    #    pan_f=pan
        ...
        ms_f = self.patchunembe(ms_f,(h,w))
        hrms = self.output(ms_f)
        return hrms
class Net_Only_Sharpening(nn.Module):
    def __init__(self,num_channels=None,base_filter=None,args=None):
        super(Net_Only_Sharpening, self).__init__()
        base_filter=32
        self.base_filter = base_filter
        self.stride=1
        self.patch_size=1
