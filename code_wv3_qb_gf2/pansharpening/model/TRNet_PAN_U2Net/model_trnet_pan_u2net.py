...
class TRNet_PAN_U2Net(nn.Module):
    ...
    def forward(self, x, y):
        ...
        if self.training:
            ...
            if self._saved_train_images_done:
                ...
                x = self.fusion_TRNet(x, K=18, pan_feat=pan_feat)
                x = self.sr(x)
    ...
    def train_step(self, data, *args, **kwargs):
        ...
        save_train_dir = r"/home/tongyufei/Pansharpen-DL-toolbox/UDL/results/image/TRNet_PAN_U2Net/train"
    ...
    def val_step(self, data, *args, **kwargs):
        ...
        #     1. 调整格式： showimage8的输入格式是[h,w,C]，当前多光谱图像ms的格式是[B,C,h,w]
        #     2. 保存图像个数： 总共只需要记录 5 张多光谱图像 ms 即可，即可以选取第一批次的前 5 张多光谱图像记录
        save_val_dir = r"/home/tongyufei/Pansharpen-DL-toolbox/UDL/results/image/TRNet_PAN_U2Net/test"
        if not self.training and not getattr(self, "_saved_train_images_done", False):
