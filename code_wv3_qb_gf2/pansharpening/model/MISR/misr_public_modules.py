import sys
import os
sys.path.append("/home/tongyufei/SatFusion")
...
class SuperResTransformer_TRMISR(nn.Module):
    def __init__(self, *, dim, depth, heads, mlp_dim, dropout=0.1):
        ...
        # 补充pan_token经过MLP之后作为cls_token
        self.msi_channels = 4
        self.dim = dim

        self.pan_proj = nn.Conv2d(self.msi_channels, dim, kernel_size=1)

        self.pan_ln = nn.LayerNorm(dim)
        self.pan_mlp = nn.Sequential(nn.Linear(dim, dim), nn.GELU(), nn.Linear(dim, dim))

        self.orignal_token = nn.Parameter(torch.randn(1, 1, dim))

        self.transformer = Transformer(dim, depth, heads, mlp_dim, dropout)
        self.to_cls_token = nn.Identity()

    def forward(self, img, pan_feat=None, mask=None, maps=None, K=64):
        b, n, _ = img.shape
        ...
        if pan_feat != None:
            # pan_feat: [B, C_pan, H, W]
            pan_proj = self.pan_proj(pan_feat)  # [B, dim, H, W]
            pan_flat = pan_proj.permute(0, 2, 3, 1).reshape(-1, self.dim)  # [B*H*W, dim]
            pan_norm = self.pan_ln(pan_flat)  # LN over last dim
            cls_token = self.pan_mlp(pan_norm).unsqueeze(1)  # [B*H*W,1,dim]

        else:
            # No need to add position code, just add token
            cls_token = repeat(self.orignal_token, '() n d -> b n d', b=b)
